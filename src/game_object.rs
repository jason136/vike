...
//         let vertex_buffer =
//             Model::create_device_local_buffer(renderer, vertices, BufferUsage::vertex_buffer());

//         let index_buffer = if indices.is_some() {
//             Some(Model::create_device_local_buffer(
//                 renderer,
//                 indices.unwrap(),
...
//         cb.execute(renderer.queue.clone())
//             .unwrap()
...
//         let cornell_box = load_obj(filepath, &tobj::GPU_LOAD_OPTIONS);
...
//             let mut uv = [0.0, 0.0];
//             if has_uvs {
//                 uv = [mesh.texcoords[index * 2 + 0], mesh.texcoords[index * 2 + 1]];
//             }
...
//         Model::new(renderer, vertices, Some(indices))
//     }
// }
